...
class Parser(object):
        ...
        while (1):
            if self.curr_token[0] == 'TK_IDENTIFIER':
                ...
                self.symtable.append(
                    {'NAME': self.curr_token[1], 'TYPE': 'empty', 'VALUE': 0, 'ADDRESS': self.address})
                self.address += 4
                self.match('TK_IDENTIFIER')
    ...
    def statements(self):
        ...
        while (1):
            ...
            if self.curr_token[0] == 'TK_SEMICOLON':
                ...
                if self.op:
                    self.instructions.append(
                        {'instruction': 'op_pop', 'ip': self.ip, 'value': self.lhs})
                    self.ip += 1
                    self.op = False
    ...
    def for_statement(self):
        ...
        loop_start = self.ip
        self.instructions.append(
            {'instruction': 'op_push', 'ip': self.ip, 'value': loop_var, 'token': 'TK_IDENTIFIER'})
        self.ip += 1
        self.factor()
        self.match('TK_DO')
        self.instructions.append(
            {'instruction': 'op_greater', 'ip': self.ip, 'value': 'greater'})
        self.ip += 1
        hole = self.ip
        self.instructions.append(
            {'instruction': 'op_jtrue', 'ip': self.ip, 'value': hole})
        self.ip += 1
        self.statements()
        ...
        self.ip += 1
        self.instructions.append(
            {'instruction': 'op_push', 'ip': self.ip, 'value': 1, 'token': 'TK_INTEGER'})
        self.ip += 1
        self.instructions.append(
            {'instruction': 'op_add', 'ip': self.ip, 'value': '+'})
        self.ip += 1
        self.instructions.append(
            {'instruction': 'op_pop', 'ip': self.ip, 'value': loop_var, 'token': 'TK_IDENTIFIER'})
        self.ip += 1
        self.instructions.append(
            {'instruction': 'op_jmp', 'ip': self.ip, 'value': loop_start})
        self.ip += 1
        self.patch(hole)
    ...
    def writeln(self):
        ...
        self.match('TK_SEMICOLON')
        self.instructions.append(
            {'instruction': 'op_writeln', 'ip': self.ip, 'value': ''})
        self.ip += 1
    ...
    def if_statement(self):
        ...
        hole_1 = self.ip
        self.instructions.append(
            {'instruction': 'op_jfalse', 'ip': self.ip, 'value': 0})
        self.ip += 1
        self.statements()
        ...
        if self.curr_token[0] == 'TK_ELSE':
            hole_2 = self.ip
            self.instructions.append(
                {'instruction': 'op_jmp', 'ip': self.ip, 'value': 0})
            self.ip += 1
            self.match('TK_ELSE')
            self.patch(hole_1)
            self.statements()
    ...
    def repeat(self):
        ...
        self.logic()
        self.instructions.append(
            {'instruction': 'op_jfalse', 'ip': self.ip, 'value': target})
        self.ip += 1
    ...
    def while_loop(self):
        ...
        self.statements()
        self.instructions.append(
            {'instruction': 'op_jmp', 'ip': self.ip, 'value': target})
        self.ip += 1
        self.patch(hole)
        return
    ...
    def postfix(self, token):
        ...
        if token[0] == 'TK_IDENTIFIER':
            self.instructions.append(
                {'instruction': 'op_push', 'value': self.curr_token[1], 'ip': self.ip, 'token': self.curr_token[0]})
            self.ip += 1
            ...
            self.instructions.append(
                {'instruction': 'op_mult', 'value': '*', 'ip': self.ip, 'token': '*'})
            self.ip += 1
        elif token == 'TK_DIV_FLOAT':
            self.instructions.append(
                {'instruction': 'op_div_float', 'value': '/', 'ip': self.ip,  'token': '/'})
            self.ip += 1
        elif token == 'TK_PLUS':
            self.instructions.append(
                {'instruction': 'op_add', 'value':  '+', 'ip': self.ip, 'token': '+'})
            self.ip += 1
        elif token == 'TK_MINUS':
            self.instructions.append(
                {'instruction': 'op_minus', 'value':  '-', 'ip': self.ip, 'token': '-'})
            self.ip += 1
        elif token == 'TK_MOD':
            self.instructions.append(
                {'instruction': 'op_mod', 'value': 'mod', 'ip': self.ip, 'token': 'TK_MOD'})
            self.ip += 1
        elif token == 'TK_OR':
            self.instructions.append(
                {'instruction': 'op_or', 'value': 'or', 'ip': self.ip, 'token': 'TK_OR'})
            self.ip += 1
        elif token == 'TK_XOR':
            self.instructions.append(
                {'instruction': 'op_xor', 'value': 'xor', 'ip': self.ip, 'token': 'TK_XOR'})
            self.ip += 1
        elif token == 'TK_AND':
            self.instructions.append(
                {'instruction': 'op_and', 'value': 'and', 'ip': self.ip, 'token': 'TK_AND'})
            self.ip += 1
        elif token == 'TK_NOT':
            self.instructions.append(
                {'instruction': 'op_not', 'value': 'not', 'ip': self.ip, 'token': 'TK_NOT'})
            self.ip += 1
        elif token == 'TK_LESS':
            self.instructions.append(
                {'instruction': 'op_less', 'value': 'less', 'ip': self.ip, 'token': 'TK_LESS'})
            self.ip += 1
        elif token == 'TK_GREATER':
            self.instructions.append(
                {'instruction': 'op_greater', 'value': 'greater', 'ip': self.ip, 'token': 'TK_GREATER'})
            self.ip += 1
        elif token == 'TK_LESS_EQUALS':
            self.instructions.append(
                {'instruction': 'op_less_equals', 'value': 'less_equals', 'ip': self.ip, 'token': 'TK_LESS_EQUALS'})
        elif token == 'TK_GREATER_EQUALS':
            self.instructions.append({'instruction': 'op_greater_equals',
                                     'value': 'greater_equals', 'ip': self.ip,  'token': 'TK_GREATER_EQUALS'})
            self.ip += 1
        elif token == 'TK_EQUALS':
            self.instructions.append(
                {'instruction': 'op_equals', 'value': 'equals', 'ip': self.ip, 'token': 'TK_EQUALS'})
            self.ip += 1
        elif token == 'TK_NOT_EQUALS':
            self.instructions.append(
                {'instruction': 'op_not_equals', 'value': 'not_equals', 'ip': self.ip, 'token': 'TK_NOT_EQUALS'})
            self.ip += 1
        else:
            pass
